...
class ContactList
  ...
  when "list"
    puts Contact.all
    puts '---------'
    puts "Total contact: #{Contact.count(:id)}"
    #TODO phone active record
  ...
  when "new"
    ...
    contact_id = Contact.create(name: full_name, email: email)
  ...
  when "search"
    unless param.nil? || param.empty?
      puts Contact.where("name LIKE ? OR email LIKE ? ", "%#{param}%", "%#{param}%")
    else
      puts "Please input a param"
    end
  when "update"
